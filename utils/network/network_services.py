...
async def network_services_kill(device: str) -> bool:
    ...
    await _handle_error(code_nm, error_nm)

    code_wpa_supplicant,_ ,error_wpa_supplicant = await run_command("systemctl", "stop", "wpa_supplicant")
    await _handle_error(code_wpa_supplicant, error_wpa_supplicant)

    code_disconnect,_, err_disconnect= await run_command("iw","dev",device, "disconnect")
    await _handle_error(code_disconnect, err_disconnect)
...
async def _handle_error(code, err) -> None:
    if code != 0:
        raise CommandException(code=code, err=err)
